...
describe("parseEsv", () => {
    test.concurrent("parse james 1:1", async () => {
    ...
    test.concurrent("parse 1 peter 1", async () => {
    ...
    test.concurrent("parse song of solomon 3", async () => {
        ...
        parseChapter(verseHtml)
    })

    test.concurrent("parse song of solomon 4", async () => {
    ...
    test.concurrent("parse james 4", async () => {
    ...
    test.concurrent("parse psalm 145:13", async () => {
...
describe("parsePrevChapter", () => {
    test.concurrent("1 peter 1", async () => {
        const prevChapter = await parsePrevChapter("1_peter", 1, "esv")
        expect(prevChapter).toEqual({ url: `james_5`, label: "James 5" })
    })

    test.concurrent("1 peter 5", async () => {
        const prevChapter = await parsePrevChapter("1_peter", 5, "esv")
        expect(prevChapter).toEqual({ url: `1_peter_4`, label: "1 Peter 4" })
    })

    test.concurrent("revelation 22", async () => {
        const prevChapter = await parsePrevChapter("revelation", 22, "esv")
    ...
    test.concurrent("genesis 1", async () => {
        const prevChapter = await parsePrevChapter("genesis", 1, "esv")
        expect(prevChapter).toBeNull()
    })

    test.concurrent("titus", async () => {
        const prevChapter = await parsePrevChapter("titus", 1, "esv")
        expect(prevChapter).toEqual({
            url: `2_timothy_4`,
            label: "2 Timothy 4",
        })
...
describe("parseNextChapter", () => {
    test.concurrent("1 peter 1", async () => {
        const nextChapter = await parseNextChapter("1_peter", 1, "esv")
        expect(nextChapter).toEqual({ url: `1_peter_2`, label: "1 Peter 2" })
    })

    test.concurrent("1 peter 5", async () => {
        const nextChapter = await parseNextChapter("1_peter", 5, "esv")
        expect(nextChapter).toEqual({ url: `2_peter_1`, label: "2 Peter 1" })
    })

    test.concurrent("revelation 22", async () => {
        const nextChapter = await parseNextChapter("revelation", 22, "esv")
        expect(nextChapter).toBeNull()
    })

    test.concurrent("genesis 1", async () => {
        const nextChapter = await parseNextChapter("genesis", 1, "esv")
        expect(nextChapter).toEqual({ url: `genesis_2`, label: "Genesis 2" })
    })

    test.concurrent("1 timothy 6", async () => {
        const nextChapter = await parseNextChapter("1_timothy", 6, "esv")
        expect(nextChapter).toEqual({
            url: `2_timothy_1`,
            label: "2 Timothy 1",
        })
    })
    test.concurrent("1 thessalonians 4", async () => {
        const nextChapter = await parseNextChapter("1_thessalonians", 4, "esv")
        expect(nextChapter).toEqual({
...
describe("Song of Solomon HTML files - no embedded newlines in words", () => {
    ...
    test.each(songOfSolomonFiles)(
        ...
        async filename => {
            ...
            const parsed = await parseChapter(content)
