...
import { Axis, LineSeries, XYChart, Tooltip, GlyphSeries, buildChartTheme } from "@visx/xychart";
import { defaultStyles as defaultTooltipStyles } from "@visx/tooltip";
import { AggregatedStats, VerseStatsWithDate, TimeRange, Interval, aggregateStats } from "./wpm";
...
function WPMChartInner({ data }: { data: AggregatedStats[] }) {
  const hasAnyData = data.some((d) => d.averageWpm !== null || d.averageAccuracy !== null);
        ...
        >
          ...
          <Tooltip
            ...
            renderTooltip={({ tooltipData }) => {
              const datum = tooltipData?.nearestDatum?.datum as AggregatedStats;
              if (!datum) return null;
              return (
                <div className="bg-secondary px-3 py-2 text-primary">
                  <div className="mb-1 text-base font-medium">{datum.dateLabel}</div>
